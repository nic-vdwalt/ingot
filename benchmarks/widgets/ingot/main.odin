...
layout_render :: proc(surface: ^fit.Surface, rect: fit.Rect, user_data: rawptr) -> bool {
	assert(surface != nil && user_data != nil, "layout_render: invalid argument")
	h := cast(^Harness)user_data
	flow: fit.Flow_State
...
benchmark_draw :: proc(builder: ^fit.Builder, user_data: rawptr) {
	assert(builder != nil && user_data != nil, "benchmark_draw: invalid argument")
	h := cast(^Harness)user_data
	switch h.workload {
	case .Labels_Repeated:
