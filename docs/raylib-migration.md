...
| 3D, models, meshes, and materials | **Import-only** for cubes and grids; **manual port** otherwise | Cube helpers and `DrawGrid` use a compatibility-owned depth target and require no caller cleanup. `DrawCubeTransform` and `DrawCubeWiresTransform` accept model matrices for rigid-body cubes. The grid lies on the ROS XY plane at Z=0. Compatibility lines, billboards, spheres, and meshes remain CPU-projected approximations; `DrawMesh` renders a shaded disc. Use the explicit GPU 3D API for custom mesh work. General model loading, skeletal animation, arbitrary `rlgl` 3D transforms, and raylib material parity remain unsupported. | [`gfx/camera.odin`](../gfx/camera.odin), [`gfx/render3d.odin`](../gfx/render3d.odin), [`gfx/gpu3d_pipeline.odin`](../gfx/gpu3d_pipeline.odin) |
