...
| Cameras | **Import-only** for `Camera2D`; **behavior change** for 3D | 3D uses ROS +X-forward, +Y-left, +Z-up. `UpdateCamera` consumes explicit free/fly motion rates and delta time; it does not poll input. Target-centered applications can instead use caller-owned `Orbit_Camera_State`, semantic input, `update_orbit_camera`, and `orbit_camera_apply`; keyboard values are rates while pointer drag and fractional wheel/trackpad scroll are frame deltas. `BeginMode3D` opens a compatibility-owned depth pass for supported primitives. `begin_mode_3d_pro` remains matrix-only for CPU projection; use `begin_gpu_3d_pro` for a custom matrix with real depth. | [`gfx/types.odin`](../gfx/types.odin), [`gfx/camera.odin`](../gfx/camera.odin), [3D contract](rendering.md#explicit-gpu-3d) |
...
| 3D, models, meshes, and materials | **Import-only** for cubes and grids; **manual port** otherwise | Cube helpers and `DrawGrid` use a compatibility-owned depth target and require no caller cleanup. `draw_cube_transform` and `draw_cube_wires_transform` accept model matrices for rigid-body cubes. The grid lies on the ROS XY plane at Z=0. Compatibility lines, billboards, spheres, and meshes remain CPU-projected approximations; `DrawMesh` renders a shaded disc. Use the explicit GPU 3D API for custom mesh work. General model loading, skeletal animation, arbitrary `rlgl` 3D transforms, and raylib material parity remain unsupported. | [`gfx/camera.odin`](../gfx/camera.odin), [`gfx/render3d.odin`](../gfx/render3d.odin), [`gfx/gpu3d_pipeline.odin`](../gfx/gpu3d_pipeline.odin) |
