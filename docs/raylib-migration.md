...
| Cameras | **Import-only** for `Camera2D`; **behavior change** for 3D | 3D uses ROS +X-forward, +Y-left, +Z-up. `UpdateCamera` consumes explicit rates and delta time; it does not poll input. `BeginMode3DPro` and `begin_gpu_3d_pro` accept caller-owned view-projection matrices. Ordinary `Camera3D` entry points remain available. Compatibility `BeginMode3D` is still CPU-projected and lacks depth. | [`gfx/types.odin`](../gfx/types.odin), [`gfx/camera.odin`](../gfx/camera.odin), [3D contract](rendering.md#explicit-gpu-3d) |
