...
if [ "${INGOT_CHECK_WEB_THREADS:-0}" = "1" ]; then
	...
	echo "== wasm compile: threaded examples/box3d_advanced =="
	INGOT_WEB_THREADS=1 bash "$ROOT/build_web.sh" examples/box3d_advanced >/dev/null
	python3 "$ROOT/scripts/check_wasm_threads.py" "$ROOT/web/ingot_web.wasm" \
		--required-export ingot_box3d_worker_command \
		--required-export ingot_box3d_advanced_set_stress
