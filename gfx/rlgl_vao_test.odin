...
rlgl_binding_state_is_context_bound :: proc(t: ^testing.T) {
	...
	first.id = 2
	second.id = 3
	first_vao := context_rl_load_vertex_array(first)
	second_vao := context_rl_load_vertex_array(second)
	defer context_rl_unload_vertex_array(first, first_vao)
	defer context_rl_unload_vertex_array(second, second_vao)

	testing.expect(t, context_rl_enable_vertex_array(first, first_vao))
	testing.expect(t, context_rl_enable_vertex_array(second, second_vao))
	testing.expect(t, !context_rl_enable_vertex_array(second, first_vao))
	context_rl_enable_inst_shader(first, 17)
	context_rl_enable_inst_shader(second, 29)
	context_rl_disable_vertex_array(first)
	context_rl_disable_inst_shader(first)
