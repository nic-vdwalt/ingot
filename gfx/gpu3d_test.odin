...
@(test)
test_gpu_3d_target_copy_compatibility_rejects_aliases :: proc(t: ^testing.T) {
	source := Gpu_3D_Target {
		texture = {
			texture = {id = 1, width = 8, height = 8},
			depth = {id = 2, width = 8, height = 8},
	...
	destination := Gpu_3D_Target {
		texture = {
			texture = {id = 3, width = 8, height = 8},
			depth = {id = 4, width = 8, height = 8},
		},
	}
	testing.expect(t, _gpu_3d_target_copy_compatible(&source, &destination))
	testing.expect(t, !_gpu_3d_target_copy_compatible(&source, &source))
	destination.texture.texture.id = source.texture.texture.id
	testing.expect(t, !_gpu_3d_target_copy_compatible(&source, &destination))
	destination.texture.texture.id = 3
	destination.texture.depth.id = source.texture.depth.id
	testing.expect(t, !_gpu_3d_target_copy_compatible(&source, &destination))
	destination.texture.depth.id = 4
	destination.texture.texture.width = 9
	testing.expect(t, !_gpu_3d_target_copy_compatible(&source, &destination))
}
...
@(test)
test_gpu_3d_atmosphere_medium_contract :: proc(t: ^testing.T) {
	pass: Gpu_3D_Pass
	uniforms := _gpu_3d_uniforms(&pass, {}, Matrix(1), false, false, false)
	testing.expect_value(t, uniforms.atmosphere_0, [4]f32{})
	testing.expect_value(t, uniforms.atmosphere_1, [4]f32{})
	testing.expect_value(t, uniforms.atmosphere_2, [4]f32{})
	medium := [3][4]f32{{0.004, 18, 1, 1}, {0.05, 1085, 2.5, 0.76}, {1, -2, 3, 8}}
	set_gpu_3d_atmosphere_medium(&pass, medium)
	uniforms = _gpu_3d_uniforms(&pass, {}, Matrix(1), false, false, false)
	testing.expect_value(t, uniforms.atmosphere_0, medium[0])
	testing.expect_value(t, uniforms.atmosphere_1, medium[1])
	testing.expect_value(t, uniforms.atmosphere_2, medium[2])
}
