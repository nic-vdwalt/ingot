...
// SMOKE (scripts/smoke-gallery.sh, -define:INGOT_SMOKE=true) turns the
// gallery into a self-driving crash harness: every SMOKE_STEP_FRAMES frames
// it advances through scale presets × themes × contrast × reduced-motion ×
// sections through the SAME handlers user input reaches (apply_scale,
// apply_gallery_theme, section switches), then exits 0. Any crash/abort
// (e.g. GPU validation) fails the script with a nonzero exit. This exists
// because the UI-scale mid-frame texture-destroy crash was reachable only
// through the real event path in a live window.
