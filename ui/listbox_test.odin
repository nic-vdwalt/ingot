...
selectable_row_requires_press_origin_and_same_row_release :: proc(t: ^testing.T) {
	...
	row := selectable_row(&frame, &state, config, {{0, 0, 100, 24}, "A", "a", 0})
	...
	ui_frame_begin(&frame, &runtime, &input)
	_ = listbox_begin(&frame, &state, config)
	other := selectable_row(&frame, &state, config, {{0, 24, 100, 24}, "B", "b", 1})
	owner := selectable_row(&frame, &state, config, {{0, 0, 100, 24}, "A", "a", 0})
	...
	ui_frame_begin(&frame, &runtime, &input)
	_ = listbox_begin(&frame, &state, config)
	_ = selectable_row(&frame, &state, config, {{0, 0, 100, 24}, "A", "a", 0})
	listbox_end(&frame, &state)
	ui_frame_end(&frame)
...
selectable_row_hover_policy_and_disabled_state :: proc(t: ^testing.T) {
	...
	ui_frame_begin(&frame, &runtime, &input)
	_ = listbox_begin(&frame, &state, config)
	row := selectable_row(&frame, &state, config, {{0, 24, 100, 24}, "B", "b", 1})
...
listbox_records_stable_collection_semantics :: proc(t: ^testing.T) {
	...
	ui_frame_begin(&frame, &runtime)
	_ = listbox_begin(&frame, &state, config)
	_ = selectable_row(&frame, &state, config, {{0, 48, 100, 24}, "C", "model:c", 2})
