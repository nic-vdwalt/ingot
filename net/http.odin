...
http_request :: proc(host: string, port: int, request: Http_Request, allocator := context.allocator) -> (Http_Response, bool) {
...
http_get_interruptible :: proc(f: ^Fetcher, idx: int, host: string, port: int, path: string, allocator := context.allocator) -> (body: []u8, ok: bool) {
	response, request_ok := http_request_impl(f, idx, host, port, Http_Request{
		method = .Get,
		path = path,
		maximum_body = DEFAULT_MAXIMUM_BODY,
	}, allocator)
...
build_request :: proc(host: string, port: int, request: Http_Request) -> []u8 {
	buf: [dynamic]u8
	buf.allocator = context.temp_allocator
	append(&buf, ..transmute([]u8)fmt.tprintf("%s %s HTTP/1.1\r\nHost: %s:%d\r\nConnection: close\r\nUser-Agent: ingot-net/0.2\r\n", method_name(request.method), request.path, host, port))
...
decode_chunked :: proc(data: []u8, maximum_body: int, allocator: mem.Allocator) -> ([]u8, bool) {
	...
	for {
		...
		if size > u64(maximum_body) || u64(len(out)) + size > u64(maximum_body) || cursor + int(size) + 2 > len(data) {
			delete(out)
			return nil, false
		}
