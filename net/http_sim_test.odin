...
test_sim_same_seed_is_identical :: proc(t: ^testing.T) {
	...
	defer { delete(first.tags); delete(first.statuses); delete(second.tags); delete(second.statuses) }
...
test_sim_different_seeds_diverge :: proc(t: ^testing.T) {
	...
	testing.expect(t, first.stats != second.stats, "distinct seeds should produce distinct fault schedules")
}
...
test_sim_faults_occur_at_high_rate :: proc(t: ^testing.T) {
	...
	faulted := record.stats.dropped + record.stats.corrupted + record.stats.errored + record.stats.truncated + record.stats.duplicated
