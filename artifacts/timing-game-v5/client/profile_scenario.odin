...
profile_scenario_frame :: proc(value: ^Client_State) {
	...
	when PROFILE_ENABLED {
		...
		if !scenario.initialized {
			...
			value.orbit.distance = f32(clamp(profile_env_number("FORGE_PROFILE_DISTANCE", f64(value.orbit.distance)), f64(CAMERA_MIN_DISTANCE), f64(CAMERA_MAX_DISTANCE)))
			scenario.paused = os.get_env("FORGE_PROFILE_PAUSED", context.temp_allocator) == "1"
		...
		if elapsed >= scenario.duration {
			...
			if len(capture_path) > 0 {
				...
				fmt.printf("Profile scene capture: %s success=%v paused=%v tick=%d sources=%v target=%v\n", capture_path, captured, value.pause.open || scenario.paused, value.tick, planet_surface_revision(&value.terrain, &value.world), value.terrain.surface_publication.target)
				fmt.printf("Profile materials: climate=%d albedo=%d active=%v rows=%d/%d uploads=%v captures=%d generations=%d\n", value.terrain.climate_row, value.terrain.albedo_row, value.terrain.surface_publication.active, value.terrain.surface_publication.order_cursor, value.terrain.surface_publication.order_count, value.terrain.upload_faces, value.terrain.surface_publication.captures, value.terrain.surface_publication.generations)
			}
