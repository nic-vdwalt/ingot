...
Client_State :: struct {
	...
	fauna:                      Fauna_Assets,
	marine_assets:              Marine_Assets,
	marine_render:              Marine_Render_State,
...
graphics_create :: proc(value: ^Client_State) -> bool {
	...
	marine_ok := marine_assets_init(&value.marine_assets)
...
game_frame :: proc(value: ^Client_State, surface: ^fit.Surface) {
	...
	if !value.pause.open && !value.profile_scenario.paused do sim_update(value, frame_dt)
...
shutdown :: proc(value: ^Client_State) {
	...
	marine_assets_deinit(&value.marine_assets)
