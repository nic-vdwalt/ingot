...
terrain_striped_albedo_bake_matches_serial :: proc(t: ^testing.T) {
	...
	for index in int(low) * PLANET_ALBEDO_SIZE ..< int(high) * PLANET_ALBEDO_SIZE {
		if striped.albedo_pixels[index * 3] != serial.albedo_pixels[index * 3] {
			testing.expectf(t, false, "albedo differs at %d", index)
			return
		}
		if striped.normal_pixels[index * 3] != serial.normal_pixels[index * 3] {
			testing.expectf(t, false, "normal differs at %d", index)
			return
		}
		if striped.roughness_ao_pixels[index * 3 + 1] !=
		   serial.roughness_ao_pixels[index * 3 + 1] {
			testing.expectf(t, false, "ao differs at %d", index)
			return
		}
		if striped.roughness_ao_pixels[index * 3 + 2] !=
		   serial.roughness_ao_pixels[index * 3 + 2] {
			testing.expectf(t, false, "snow mask differs at %d", index)
			return
		}
	}
}
