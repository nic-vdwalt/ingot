...
_albedo_scratch_fill :: proc(
	...
	for offset, tap in taps {
		...
		for column in 0 ..< PLANET_ALBEDO_SIZE {
			delta := _face_delta_bilinear(world, face, _albedo_texel_cell(i32(column)), cell_v)
			heights[column] = base[column] + delta
...
_albedo_bake_row_scratch :: proc(
	...
	for column in i32(0) ..< i32(PLANET_ALBEDO_SIZE) {
		...
		slope_x := (right - left) / (2 * step_u)
		slope_y := (up - down) / (2 * step_v)
