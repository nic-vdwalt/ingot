...
marine_snapshot_body :: proc(state: ^Marine_Ecology, buffer: []u8, reading: bool) -> bool {
	...
	if reading do state.mutation_enabled, state.frozen = flags[0] != 0, flags[1] != 0
	...
	for &cell in state.cells {
		...
		for &cohort in cell.cohorts {
			...
			if reading do cohort.lineage = u32(identity)
...
marine_ecology_snapshot_read :: proc(state: ^Marine_Ecology, buffer: []u8, allocator: mem.Allocator = {}) -> bool {
	...
	owner := allocator
	if owner.procedure == nil do owner = state.allocator
	if owner.procedure == nil do owner = context.allocator
	temporary: Marine_Ecology
	if !marine_ecology_init(&temporary, 0, owner, int(header[1])) do return false
