...
world_snapshot_read :: proc(world: ^World, buffer: []u8) -> bool {
	...
	cursor += size_of(u64)
	marine_start := flora_start - int(marine_payload_size)
	staged_marine: Marine_Ecology
	if !marine_ecology_snapshot_read(&staged_marine, buffer[marine_start:flora_start], world.allocator) do return false
	defer marine_ecology_deinit(&staged_marine)
	if staged_marine.step == 0 {
		if staged_marine.birth_serial != 0 || staged_marine.local_extinctions != 0 || staged_marine.global_extinctions != 0 || staged_marine.suppressed_mutations != 0 do return false
		for lineage in staged_marine.lineages[:staged_marine.lineage_count] do if lineage.occupied_steps != 0 do return false
		marine_initial_diagnostics(&staged_marine)
	}
	if len(staged_marine.cells) != PLANET_SIM_CELL_COUNT || !marine_diagnostics_valid(&staged_marine) do return false
	...
	world.marine_ecology, staged_marine = staged_marine, world.marine_ecology
