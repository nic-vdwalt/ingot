...
host CPU p50 12.4–12.8 ms. There is no candidate build; no 120 Hz claim.

## Symptom table

| Symptom | Owning layer | Evidence | Fix or limitation | Regression | Artifact |
|---|---|---|---|---|---|
| Reversed / zero GPU pass ends in the game's window pass | Pinned wgpu-hal Metal topology on Apple M2 Max / macOS 15.6.1: same-command-buffer `resolveCounters` returns the previous pass's end-of-fragment sample; start-of-vertex is current | Exact replay of v10 frame 11: pinned WebGPU 286–297/300 reversed, native identical topology 288–298/300 reversed, native render-completed resolve 300/300 ordered; stale end equals previous pass end within 50 us | Evidenced native limitation; no production repair (CPU wait, clamp, vertex-end substitution and dummy work refused). Target publishes `rl.g = unreliable` with reason; Aesir rejects attribution | `evaluate_replay.py candidate_passes`; `test_replay_attribution.py`; Aesir `telemetry_parse_rejects_gpu_timing_without_reliable_scope` | `artifacts/timing-replay-v10-f1/` (bundle, binaries, five runs, `evaluation.json`, `attribution-manifest.json`) |
...
| Ocean pass timing | Unknown | No ocean category was recorded in any capture (v6–v11 window only) | Not attributable from window evidence; needs its own bounded capture and replay | none | reported as uncovered |
| Sustained 120 Hz presentation | Unmeasured against a candidate | Control: p50 8.33 ms, mean 16.4–16.9 ms, p95 50–58 ms; about half of gameplay deadline samples missed; host CPU p50 12.4–12.8 ms | No candidate exists; performance work starts from this control with fixed 2560x1440 world targets and scale 1 | `control-three-runs.json` | `artifacts/timing-game-v11/` |
